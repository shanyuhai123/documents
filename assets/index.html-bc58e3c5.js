import{_ as o,M as p,p as c,q as u,Q as n,t as s,N as t,a1 as e}from"./framework-49860b1b.js";const l="/assets/traefik-architecture-overview-2ef4ae24.png",r={},i=e('<h1 id="从-nginx-切换到-traefik" tabindex="-1"><a class="header-anchor" href="#从-nginx-切换到-traefik" aria-hidden="true">#</a> 从 Nginx 切换到 Traefik</h1><h2 id="为什么选择-traefik" tabindex="-1"><a class="header-anchor" href="#为什么选择-traefik" aria-hidden="true">#</a> 为什么选择 Traefik</h2><p>如果追求极致的性能那自然选择 Nginx，但对于个人而言 Traefik 也是一个良好的选择，尤其是其服务发现的能力很好的解决了前端、后端应用增加、减少时需修改配置文件的问题。</p><h2 id="traefik-的架构" tabindex="-1"><a class="header-anchor" href="#traefik-的架构" aria-hidden="true">#</a> Traefik 的架构</h2><p><img src="'+l+`" alt="traefik architecture overview"></p><p>由图所示，Traefik 会监听入口点（<code>EntryPoints</code>），路由器（<code>Routers</code>）连接到这些入口点，分析传入的请求，以查看它们是否与一组规则（<code>Rule</code>）匹配。如果匹配，则有中间件先经过中间件（<code>Middlewares</code>），再转发给对应的服务（<code>Services</code>）。</p><p>其中配置与 <code>Nginx</code> 相似，分为两部分：</p><ul><li>静态配置：<code>traefik.toml</code>，与 <code>nginx.conf</code> 类似</li><li>动态配置：<code>config/*.toml</code>，与 <code>conf.d/*.conf</code> 类似</li></ul><h2 id="以-whoami-为示例" tabindex="-1"><a class="header-anchor" href="#以-whoami-为示例" aria-hidden="true">#</a> 以 whoami 为示例</h2><p>先创建 <code>docker-compose.yml</code> 文件，静态配置（<code>traefik.toml</code>）直接记录在容器启动配置中可以带来更好的体验：</p><div class="language-yaml" data-ext="yml"><pre class="language-yaml"><code><span class="token key atrule">version</span><span class="token punctuation">:</span> <span class="token string">&#39;3.7&#39;</span>
...
</code></pre></div><p>可为动态配置抽象一些会公共使用的，增加 <code>config.default.toml</code> 文件。</p><h3 id="_1-基础认证" tabindex="-1"><a class="header-anchor" href="#_1-基础认证" aria-hidden="true">#</a> 1. 基础认证</h3>`,13),k={href:"https://doc.traefik.io/traefik/middlewares/basicauth/",target:"_blank",rel:"noopener noreferrer"},d={href:"http://httpd.apache.org/docs/1.3/programs/htpasswd.html",target:"_blank",rel:"noopener noreferrer"},m=e(`<div class="language-bash" data-ext="sh"><pre class="language-bash"><code>htpasswd <span class="token parameter variable">-nb</span> yourname yourpassword
...
<span class="token key atrule">networks</span><span class="token punctuation">:</span> 
  <span class="token key atrule">traefik</span><span class="token punctuation">:</span>
    <span class="token key atrule">external</span><span class="token punctuation">:</span> <span class="token boolean important">true</span>
</code></pre></div><h2 id="参考资料" tabindex="-1"><a class="header-anchor" href="#参考资料" aria-hidden="true">#</a> 参考资料</h2>`,15),f={href:"https://doc.traefik.io/traefik/",target:"_blank",rel:"noopener noreferrer"},g={href:"https://doc.traefik.io/traefik/reference/static-configuration/overview/",target:"_blank",rel:"noopener noreferrer"},h={href:"https://github.com/toml-lang/toml",target:"_blank",rel:"noopener noreferrer"},q={href:"https://soulteary.com/tags/traefik.html",target:"_blank",rel:"noopener noreferrer"};function y(b,v){const a=p("ExternalLinkIcon");return c(),u("div",null,[i,n("p",null,[s("基础认证采取简单的 "),n("a",k,[s("BasicAuth"),t(a)]),s("。")]),n("p",null,[s("利用 "),n("a",d,[s("htpasswd"),t(a)]),s(" 工具生成密码：")]),m,n("ol",null,[n("li",null,[n("a",f,[s("Traefik"),t(a)])]),n("li",null,[n("a",g,[s("Traefik 配置"),t(a)])]),n("li",null,[n("a",h,[s("toml"),t(a)])]),n("li",null,[n("a",q,[s("苏洋 Tag#Traefik"),t(a)])])])])}const _=o(r,[["render",y],["__file","index.html.vue"]]);export{_ as default};
