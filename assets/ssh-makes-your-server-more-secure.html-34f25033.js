import{_ as p,M as t,p as r,q as l,Q as a,t as s,N as n,V as i,a1 as e}from"./framework-49860b1b.js";const d={},h=a("h1",{id:"ssh-让你的服务器更安全",tabindex:"-1"},[a("a",{class:"header-anchor",href:"#ssh-让你的服务器更安全","aria-hidden":"true"},"#"),s(" SSH 让你的服务器更安全")],-1),u=a("p",null,"安全是相对而言的，正在不断探索中。",-1),m=a("h2",{id:"ssh-架构",tabindex:"-1"},[a("a",{class:"header-anchor",href:"#ssh-架构","aria-hidden":"true"},"#"),s(" SSH 架构")],-1),k={href:"https://en.wikipedia.org/wiki/Secure_Shell_Protocol",target:"_blank",rel:"noopener noreferrer"},b=a("strong",null,"Secure Shell Protocol",-1),g=a("strong",null,"SSH",-1),_=e(`<ul><li>客户端(ssh)：全局配置路径为 <code>/etc/ssh/ssh_config</code>，当前用户为 <code>~/.ssh</code>。</li><li>服务端(sshd)：全局配置路径为 <code>/etc/ssh/sshd_config</code>，以及对应用户的 <code>~/.ssh/authorized_keys</code>。</li></ul><h3 id="客户端" tabindex="-1"><a class="header-anchor" href="#客户端" aria-hidden="true">#</a> 客户端</h3><p>访问服务器，第一次访问服务器会出现认证提示：</p><div class="language-bash" data-ext="sh"><pre class="language-bash"><code><span class="token comment"># user 为用户名</span>
...
</code></pre></div><h3 id="ssh-隧道" tabindex="-1"><a class="header-anchor" href="#ssh-隧道" aria-hidden="true">#</a> SSH 隧道</h3>`,24),v={href:"https://en.wikipedia.org/wiki/Port_forwarding",target:"_blank",rel:"noopener noreferrer"},f=e(`<ul><li>本地端口转发（Local port forwarding）</li><li>远程端口转发（Remote port forwarding）</li><li>动态端口转发（Dynamic port forwarding）</li></ul><p>我们需要在本地访问部署在服务器上的服务，选择本地端口转发即可。例如在服务器的 30000 端口部署了 Web 服务。</p><p>可通过 <code>man ssh</code> 查看语法：</p><div class="language-bash" data-ext="sh"><pre class="language-bash"><code><span class="token parameter variable">-L</span> <span class="token punctuation">[</span>bind_address:<span class="token punctuation">]</span>port:host:hostport
...
</code></pre></div>`,3);function y(S,w){const o=t("ExternalLinkIcon"),c=t("RouterLink");return r(),l("div",null,[h,u,m,a("p",null,[a("a",k,[b,s(" ("),g,s(")"),n(o)]),s(" 分为两个部分，默认安装：")]),_,a("p",null,[s("可以使用 SSH 隧道（SSH Tunnel）的"),a("a",v,[s("转发模式"),n(o)]),s("有三种：")]),f,a("p",null,[s("如果有黑客持续爆破你的服务器，那么可以使用 "),n(c,{to:"/os/linux/fail2ban.html"},{default:i(()=>[s("fail2ban")]),_:1}),s(" 来限制他的 IP。")]),x])}const L=p(d,[["render",y],["__file","ssh-makes-your-server-more-secure.html.vue"]]);export{L as default};
