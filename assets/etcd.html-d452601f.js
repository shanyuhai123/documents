import{_ as o,M as l,p as c,q as r,a1 as e,R as n,t as s,N as p}from"./framework-e3e34937.js";const i={},d={href:"https://etcd.io/docs/v3.4.0/dev-internal/discovery_protocol/",target:"_blank",rel:"noopener noreferrer"},k={href:"https://github.com/etcd-io/etcd/issues/11321",target:"_blank",rel:"noopener noreferrer"};function u(b,a){const t=l("ExternalLinkIcon");return c(),r("div",null,[a[8]||(a[8]=e('<h1 id="使用-docker-搭建-etcd" tabindex="-1"><a class="header-anchor" href="#使用-docker-搭建-etcd" aria-hidden="true">#</a> 使用 Docker 搭建 ETCD</h1><h2 id="etcd-基础概念" tabindex="-1"><a class="header-anchor" href="#etcd-基础概念" aria-hidden="true">#</a> ETCD 基础概念</h2><ul><li>Raft：etcd 所采用的保证分布式系统强一致性的算法</li><li>Node：一个 Raft 状态实例机</li><li>Member：一个 etcd 实例。它管理着一个 Node，并且可以为客户端请求提供服务</li><li>Cluster：由多个 Member 构成可以协同工作的 etcd 集群</li><li>Peer：对同一个 etcd 集群中的另外一个 Member 的称呼</li><li>Client：向 etcd 集群发送 HTTP 请求的客户端</li><li>WAL：预写式日志，etcd 用于持久化存储的日志格式</li><li>snapshot：etcd 防止 WAL 文件过多而设置的快照，存储 etcd 数据状态</li><li>Proxy：etcd 的一种模式，为 etcd 集群提供反向代理服务</li><li>Leader：Raft 算法中通过竞选而产生的处理所有数据的节点</li><li>Follower：竞选失败的节点作为 Raft 中的从属节点，为算法提供强一致性保证</li><li>Candidate：当 Follower 超过一定时间接收不到 Leader 的心跳时转变为 Candidate 开始竞选</li><li>Term：某个节点成为 Leader 到下一次竞选时间，称为一个 Term</li><li>Index：数据项编号。Raft 中通过 Term 和 Index 来定位数据</li></ul><h2 id="部署-etcd" tabindex="-1"><a class="header-anchor" href="#部署-etcd" aria-hidden="true">#</a> 部署 ETCD</h2>',4)),n("p",null,[a[1]||(a[1]=s("在部署时，不确定其他成员 IP 是常见的，可以使用")),n("a",d,[a[0]||(a[0]=s("动态发现")),p(t)]),a[2]||(a[2]=s("的方式解决。"))]),a[9]||(a[9]=e(`<h3 id="准备" tabindex="-1"><a class="header-anchor" href="#准备" aria-hidden="true">#</a> 准备</h3><p>拉取镜像：</p><div class="language-bash" data-ext="sh"><pre class="language-bash"><code><span class="token function">docker</span> pull bitnami/etcd:3.4.14
...
</code></pre></div>`,21)),n("p",null,[a[4]||(a[4]=s("此处仅以 ")),a[5]||(a[5]=n("code",null,"node1",-1)),a[6]||(a[6]=s(" 示例，注 etcd 新版本")),n("a",k,[a[3]||(a[3]=s("需要 HTTPS")),p(t)]),a[7]||(a[7]=s("，所以切换为旧版："))]),a[10]||(a[10]=e(`<div class="language-bash" data-ext="sh"><pre class="language-bash"><code><span class="token function">docker</span> stop etcd <span class="token operator">&amp;&amp;</span> <span class="token function">docker</span> <span class="token function">rm</span> etcd
