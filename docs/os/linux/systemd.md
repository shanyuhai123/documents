...
```bash
# 有些软件支持 Systemd，安装时，会自动在 `/usr/lib/systemd/system` 目录添加一个配置文件
# 然后就可以让其开机自启了
...
Systemd 并不是一个命令，而是一组命令，涉及系统管理的方方面面。
...
[原文：Systemd 入门教程](http://www.ruanyifeng.com/blog/2016/03/systemd-tutorial-commands.html)